...
void td4_q5_print_dominators(function * fun)
{
	printf("DOMINATORS:\n");
	...
	FOR_ALL_BB_FN(bb, fun)
	{
		...
		printf("Basic block BB %2d dominates (%2d):", bb->index, size-1);
		...
		for (i = 0; i<size; i++)
		{
			if (dominators[i]->index == bb->index) continue;
			printf(" { BB %2d }", dominators[i]->index);
		}
		printf("\n");
	}
